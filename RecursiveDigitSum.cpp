...
// SOLUTION - Further optimization should be explored even though it passes the hackerrank problem.
int superDigitRcs(long n)
{
    if(n < 10) 
    {
        return n;
...
int superDigit(string n, int k) {
    ...
    for(int j = 0; j < k; j++)
    {
        n_tmp += superDigitRcs(n_igr);
    }
    
    int rst =  superDigitRcs(n_tmp);
    return rst;
}
